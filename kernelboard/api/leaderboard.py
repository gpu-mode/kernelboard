...
BLOCKED_CODE_LEADERBOARD_IDS = [598]
    ...
    with conn.cursor() as cur:
        cur.execute(
            query,
            {
                ...
                "blocked_code_leaderboard_ids": BLOCKED_CODE_LEADERBOARD_IDS,
            },
        )
        result = cur.fetchone()
    ...
    query = """
        ...
        leaderboard_info AS (
            SELECT
                ...
                (
                    deadline < NOW()
                    AND NOT (id = ANY(%(blocked_code_leaderboard_ids)s::int[]))
                ) AS show_solution_code_metadata
