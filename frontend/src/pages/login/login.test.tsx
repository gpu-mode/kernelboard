...
function renderLogin(initialUrl = "/kb/login") {
  return render(
...
describe("<Login />", () => {
  it("renders Discord login button with correct href", () => {
    ...
    expect(discordBtn).toHaveAttribute("href", "/api/auth/discord?next=/kb/");
  });

  it("shows error when URL has error and message", async () => {
    renderLogin("/kb/login?error=BadAuth&message=nope");
